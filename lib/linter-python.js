...
class LinterPython extends React.Component {
    ...
    initialPlugin() {
        ...
        let enablePep8 = this.readConfigValue('enablePep8');
        if (enablePep8) {
            linters.push('pep8');
        }

        let enablePep257 = this.readConfigValue('enablePep257');
        if (enablePep257) {
            linters.push('pep257');
        }

        let enableMcCabe = this.readConfigValue('enableMcCabe');
        if (enableMcCabe) {
            linters.push('mccabe');
        }

        let enablePyflakes = this.readConfigValue('enablePyflakes');
        if (enablePyflakes) {
            linters.push('pyflakes');
        }

        let enablePylint = this.readConfigValue('enablePylint');
        if (enablePylint) {
            linters.push('pylint');
        }
