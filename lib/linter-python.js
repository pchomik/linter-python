...
class LinterPython {
    ...
    lint(textEditor) {
        ...
        return new Promise((resolve) => {
            ...
            if (this.config.shellCommand) {
                args = this.buuildShellArgs(this.config.shellCommand, cmd, args);
                cmd = this.buildShellCmd(this.config.shellCommand);
            }
    ...
    buildShellCmd(shellCommand) {
        let parts = shellCommand.split(' ');
        return parts[0];
    }

    buuildShellArgs(shellCommand, execPath, execArgs) {
        let args = [];
        let parts = shellCommand.split(' ');
        if (parts.length >= 2) {
            args.push(parts[1]);
        }
        args.push(execPath + ' ' + execArgs.join(' '));
        return args;
    }
